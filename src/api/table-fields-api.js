...
const TableFieldsAPI = {
    updateTableFields: ({ datasourceId, schemaName, tableName, fields: tableFields = [] }) => {
        return loadingCounterWrapper(async () => {
            try {
                const response = (await axios.put(`/api/v1/table-fields`, { datasourceId, tableName, schemaName, tableFields })).data;
